...
function actualizarHorarios() {
    ...
    // Recargar turnos del localStorage para asegurar sincronización
    turnosReservados = JSON.parse(localStorage.getItem('turnosReservados')) || [];

    // Mostrar todos los horarios con indicación de disponibilidad
    horariosDisponibles.forEach(hora => {
        ...
        const option = document.createElement('option');
        option.value = turnoExiste ? '' : hora; // Si está ocupado, no se puede seleccionar
    ...
    const horariosLibres = horariosDisponibles.filter(hora => {
        return !turnosReservados.some(turno =>
            turno.barbero === barberoSeleccionado &&
            turno.fecha === fechaSeleccionada &&
            turno.hora === hora
        );
    ...
    if (horariosLibres.length === 0) {
        // Agregar mensaje si no hay horarios disponibles
        const option = document.createElement('option');
